...
class EmployeeList {
    ...
    // ㅓㅇㅂ캐스팅을 자유롭게 수행될 수 있기에 Employee가 Manager 객체를 가리켜도 문제 없다.
    int current_employee;      // 현재 직원 수
    Employee** employee_list;  // 직원 데이터
...
public:
    ...
    void print_employee_info() {
        ...
        for (int i = 0; i < current_employee; i++) {
            // 무조건 Employee 클래스의 print_info 함수가 호출된다.
            employee_list[i]->print_info();
            // 마찬가지로 Employee 클래스의 calculate_pay 함수가 호출된다.
