...
// MyArray 이름 공간으로 감쌌기 때문에 다른 라이브러리에서 Array 클래스 정의해도 문제 없음
namespace MyArray {
    class Array;
    class Int;

    class Array {
        // friend 선언 전 class Int를 먼저 선언해 사용할 수 있도록 해야한다.
        friend Int;
    ...
    // Array 클래스 안에 넣지 않고 따로 빼 놓은 이유는  Int 클래스의 생성자를 사용하기 위해서 Int 클래스 정의가 선행 되어야한다.
